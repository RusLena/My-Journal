...
function Food() {
  ...
  const handleSave = () => {
    ...
    if (editingIndex === null) {
      ...
      const updatedList = foodList.map((item, index) =>
        index === editingIndex ? newFood : item
      );
      setFoodList(updatedList);
      localStorage.setItem("foodList", JSON.stringify(updatedList));
  ...
  const handleEdit = (index) => {
    const itemToEdit = foodList[index];
    setDay(itemToEdit.day);
    setDate(itemToEdit.date);
    setMealTime(itemToEdit.meal);
    setFoodType(itemToEdit.foodType);
    setOtherFoodType(itemToEdit.otherFoodType || "");
  ...
  return (
    <div
      className="page-background food-page"
      style={{
